...
public class WorkingDaysHoursValidator {
    ...
    public void validate(EventType eventType, Map<Integer,List<TimeRange>> workingDaysHours) {

        if(eventType != EventType.BUSINESS) {
            return;
        }

        if(workingDaysHours == null || workingDaysHours.isEmpty()){
            throw new AppException("Working days hours cannot be empty", HttpStatus.BAD_REQUEST);
        }

        for (Map.Entry<Integer,List<TimeRange>> entry : workingDaysHours.entrySet()){
    ...
    private void validateRanges(List<TimeRange> ranges) {
        ...
        for(int i = 1; i <times.size(); i++){
            if(!times.get(i)[0].isAfter(times.get(i-1)[1])) {
                throw new AppException("Overlapping working hours detected", HttpStatus.BAD_REQUEST);
            }
        }
    }
